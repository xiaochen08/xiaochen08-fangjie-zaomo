...
- shader compatibility contract: validated `shader-contract.json` with the baseline fallback, exact named targets, material maps, render layer, test matrix, and evidence status.
...
If `model-spec.json` declares `shader_compatibility.required: true`, the bundle must include a valid `shader-contract.json`. Include every texture/map it names. Static validity does not authorize a shader-support claim: named targets remain unverified until their applicable in-game matrix rows pass with exact versions and contained hashed evidence.
